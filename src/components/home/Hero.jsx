...
export default function Hero() {
  ...
  return (
    <section className="relative bg-navy overflow-hidden pt-16 md:pt-24 lg:pt-28">
      {/* Blueprint grid */}
      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-14 items-center">
          ...
          <div>
            ...
            <motion.div {...fade(0.1)}
              className="inline-flex items-center gap-2.5 bg-accent/[0.13] border border-accent/28
                         rounded-full px-4 py-2 mb-5 md:mb-7"
            >
            ...
            {/* H1 */}
            <motion.h1 {...fade(0.2)}
              className="text-3xl sm:text-4xl md:text-5xl xl:text-[3.4rem] font-black text-white
                         leading-[1.12] mb-4 md:mb-5"
            >
            ...
            {/* Sub */}
            <motion.p {...fade(0.3)}
              className="text-[0.97rem] md:text-[1.04rem] text-white/70 leading-[1.78] mb-6 md:mb-9 max-w-[490px]"
            >
            ...
            {/* CTAs */}
            <motion.div {...fade(0.4)} className="flex flex-wrap gap-3 md:gap-3.5 mb-7 md:mb-10">
              <Link to="/contact-us" className="btn btn-orange px-6 md:px-8 py-3 md:py-3.5 text-[0.9rem] md:text-[0.95rem]">
                Get a Free Quote →
              </Link>
              <Link to="/services" className="btn btn-outline-white px-6 md:px-8 py-3 md:py-3.5 text-[0.9rem] md:text-[0.95rem]">
                View Our Services
              </Link>
            </motion.div>

            {/* Trust flags */}
            <motion.div {...fade(0.5)} className="flex items-center gap-3.5 pb-8 md:pb-0">
          ...
          >
            <div className="rounded-xl overflow-hidden shadow-[0_32px_80px_rgba(0,0,0,0.45)] relative h-[280px] md:h-[320px] lg:h-[390px]">
              <AnimatePresence>
                <motion.img
                  key={current}
                  src={SLIDES[current]}
            ...
            {/* Float cards — only visible on lg+ where there's space */}
            <motion.div
              initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.7, duration: 0.5 }}
              className="hidden lg:block absolute -bottom-5 -left-5 bg-white rounded-xl px-5 py-3.5
                         shadow-float"
            >
            ...
            <motion.div
              ...
              className="hidden lg:block absolute top-5 -right-5 bg-white rounded-xl px-5 py-3.5
                         shadow-float"
            >
      ...
      {/* Stats bar */}
      <div className="mt-12 md:mt-20 bg-black/25 border-t border-white/[0.08] relative z-10">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4">
            {stats.map((s, i) => (
              <motion.div
                ...
                className={`py-5 md:py-8 text-center px-4 md:px-6
                  ${i < stats.length - 1 ? 'border-r border-white/[0.1]' : ''}
                  ${i === 1 ? 'border-r-0 lg:border-r border-white/[0.1]' : ''}
                `}
              >
                <div className="text-2xl md:text-3xl lg:text-4xl font-black text-accent leading-none mb-1.5 md:mb-2
                                [font-size:clamp(1.25rem,4vw,2.4rem)]">
                  {s.value}
                </div>
                <div className="text-[0.72rem] md:text-[0.78rem] text-white/55 font-medium">{s.label}</div>
              </motion.div>
            ))}
          </div>
        </div>
